...
//            months.add(DateUtils.fillZero(i) );
            months.add(DateUtils.fillZero(i) + "月");
        }
        for (int i = 1; i <= 31; i++) {
//            days.add(DateUtils.fillZero(i));
            days.add(DateUtils.fillZero(i) + "日");
        }
    }
...
//                    int maxDays = DateUtils.calculateDaysInMonth(stringToYearMonthDay(item), stringToYearMonthDay(months.get(selectedMonthIndex)));
                    int maxDays = DateUtils.calculateDaysInMonth(stringToYearMonthDay(item, 0), stringToYearMonthDay("", selectedMonthIndex + 1));
...
//                    int maxDays = DateUtils.calculateDaysInMonth(stringToYearMonthDay(years.get(selectedYearIndex)), stringToYearMonthDay(item));
                    int maxDays = DateUtils.calculateDaysInMonth(stringToYearMonthDay(years.get(selectedYearIndex), 0), stringToYearMonthDay("", selectedIndex + 1));
                    for (int i = 1; i <= maxDays; i++) {
                        days.add(DateUtils.fillZero(i) + "日");
                    }
                    if (selectedDayIndex >= maxDays) {
    ...
    private int stringToYearMonthDay(String text, int txt) {
        if ("".equals(text)) {
            return txt;
